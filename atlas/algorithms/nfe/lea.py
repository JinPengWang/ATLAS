"""Love Evolution Algorithm (LEA) -- NFE-based variant."""
...
@register_algorithm("lea_nfe")
class LEA_NFE(BaseNFEAlgorithm):
    """Love Evolution Algorithm with function-evaluation-based stopping."""
    ...
    def _eval_index(self, idx: int, candidate: np.ndarray) -> None:
        self.population[idx] = self._clip(candidate)
        self.fitness[idx] = self._evaluate(self.population[idx])
        self._update_global_best()
        ...
        for pair_idx in range(self.pop_size // 2):
            ...
            if p[pair_idx] < 0.5:
                ...
                for j in range(self.dim):
                    ...
                    c1[j] = self.rng.random() * x1[j] + self.rng.normal() * np.sqrt(abs(C2 - C1))
                    c2[j] = self.rng.random() * x2[j] + self.rng.normal() * np.sqrt(abs(C2 - C3))
                self._eval_index(idx1, c1)
                if self._nfe >= self.max_nfe:
                    break
                self._eval_index(idx2, c2)
                p[pair_idx] = (
                    (self.rng.random() + 0.5)
                    * p[pair_idx]
